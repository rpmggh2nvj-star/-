...
(function (root, factory) {
  ...
  function buildBets(rows, budget){
    ...
    /* 推定期待値が1を割る点は買わない。
       ここを残すと、当たりやすいだけで割の悪い点（人気どうしの組み合わせ）が
       必ず混ざる。単勝で「本命だから買う」をやめたのと同じ理屈である。 */
    const keepGood = (pts, floor) => {
      const lim = floor == null ? BUY_EV : floor;
      const good = pts.filter(pt => pt.ev == null || pt.ev >= lim);
      return {kept: good, cut: pts.length - good.length};
    };
    ...
    if(places){
      ...
      const mkHit = topKProb(mkts, ai, places);
      // 当たりやすくても割が悪ければ買わない。人気馬の複勝はここで落ちる。
      if(hit >= 0.5 && comboEv(hit, mkHit, "fuku") >= BUY_EV){
        push({name: `複勝（${places}着まで）`, prio: 85, ratio: 0.12,
              combos: [String(a)], hit: hit, needOdds: need(hit, 1),
              evEst: comboEv(hit, mkHit, "fuku"),
              points: withPoints([String(a)], [hit], [mkHit], "fuku"),
    ...
    /* --- 相手の広げ方は荒れ度で決める（堅い2頭／やや荒れ3頭／荒れ5頭） ---
       ...
       推定勝率の高い順に取るだけでは、相手はほぼ人気上位で埋まる。
       それでは「人気薄が絡んで荒れた」レースを構造的に取りこぼす。
       荒れると読んだレースでは枠の一部を空けて、期待値の高い人気薄を入れる。 */
    const v = verdictOf(rows);
    const others = rows.filter(x => x.h.num !== a);
    let mateRows;
    if(up.level === "low" || others.length <= up.width){
      mateRows = others.slice(0, up.width);
    }else{
      const keep = Math.max(2, up.width - 2);              // 上位は必ず残す
      const head = others.slice(0, keep);
      const dark = others.slice(keep)
        .filter(x => x.prob >= 0.015 && isValue(x))        // 割の良い人気薄だけ
        .sort((x, y) => y.ev - x.ev)
        .slice(0, up.width - keep);
      mateRows = head.concat(dark);
      // 人気薄の候補が足りなければ、確率順で埋める
      others.forEach(x => {
        if(mateRows.length < up.width && mateRows.indexOf(x) < 0) mateRows.push(x);
      });
      mateRows = mateRows.slice(0, up.width);
    }
    const mates = mateRows.map(x => x.h.num);
    const darkMates = mateRows.filter(x => x.rank > Math.max(2, up.width - 2)).map(x => x.h.num);

    if(mates.length){
      const combos = mates.map(x => `${a}-${x}`);
      const each = mates.map(m => quinellaProb(probs, idx[a], idx[m]));
      const mk   = mates.map(m => quinellaProb(mkts,  idx[a], idx[m]));
      const g = keepGood(withPoints(combos, each, mk, "umaren"));
      const hit = g.kept.reduce((s, pt) => s + pt.hit, 0);
      push({name: "馬連 流し", prio: 80, ratio: 0.20,
            combos: g.kept.map(pt => pt.combo),
            hit: hit, needOdds: need(hit, g.kept.length),
            points: g.kept, cut: g.cut,
            memo: `${a} 軸。的中 ${(hit*100).toFixed(1)}%${axisNote}` +
                  (g.cut ? `（割の悪い ${g.cut}点 を外しました）` : "")});
    }

    // ワイド・三連複は出走4頭以上でないと発売されない
    if(mates.length >= 2 && n >= 4){
      const w = mates.slice(0, up.level === "high" ? 3 : 2);
      const combos = w.map(x => `${a}-${x}`);
      const each = w.map(m => wideProb(probs, idx[a], idx[m]));
      const mk   = w.map(m => wideProb(mkts,  idx[a], idx[m]));
      const g = keepGood(withPoints(combos, each, mk, "wide"));
      const hit = g.kept.reduce((s, pt) => s + pt.hit, 0);
      push({name: "ワイド", prio: 70, ratio: up.level === "high" ? 0.22 : 0.13,
            combos: g.kept.map(pt => pt.combo),
            hit: hit, needOdds: need(hit, g.kept.length),
            points: g.kept, cut: g.cut,
            memo: `取りこぼしを減らす。的中 ${(hit*100).toFixed(1)}%` +
                  (g.cut ? `（割の悪い ${g.cut}点 を外しました）` : "")});
    }
    ...
    if(mates.length >= 3 && n >= 4 && up.level !== "low"){
      ...
      const mk   = pairs.map(pr => trioProb(mkts,  idx[a], idx[pr[0]], idx[pr[1]]));
      const g = keepGood(withPoints(tri, each, mk, "sanrenpuku"));
      const hit = g.kept.reduce((s, pt) => s + pt.hit, 0);
      push({name: "三連複 軸1頭流し", prio: 60, ratio: up.level === "high" ? 0.28 : 0.20,
            combos: g.kept.map(pt => pt.combo),
            hit: hit, needOdds: need(hit, g.kept.length),
            points: g.kept, cut: g.cut,
            memo: `${a} 軸。的中 ${(hit*100).toFixed(1)}%` +
                  (g.cut ? `（割の悪い ${g.cut}点 を外しました）` : "")});
    }

    /* --- 三連単 ---
       着順まで当てるので当たりにくいが、その分オッズが大きく、
       人気の並びが買われすぎる（＝人気薄が絡む並びは割がよい）券種でもある。
       ここは「当たりやすい順」ではなく「推定期待値の高い順」に取る。
       当たりやすさで選ぶと人気どうしの並びばかりになり、必ず割負けする。

       軸は1着か2着に置く。3着まで落とすと点数が跳ね上がるわりに拾えない。 */
    if(n >= 4 && mates.length >= 2){
      const cand = [];
      const seen = {};
      mates.forEach(m1 => mates.forEach(m2 => {
        if(m1 === m2) return;
        [[a, m1, m2], [m1, a, m2]].forEach(o => {
          const key = o.join("-");
          if(seen[key]) return;
          seen[key] = 1;
          const pm = orderProb(probs, idx[o[0]], idx[o[1]], idx[o[2]]);
          const pk = orderProb(mkts,  idx[o[0]], idx[o[1]], idx[o[2]]);
          cand.push({combo: key, hit: pm, needOdds: pm > 0 ? 1/pm : Infinity,
                     ev: comboEv(pm, pk, "sanrentan")});
        });
      }));
      /* 三連単の推定は誤差が大きいので、1.0ぎりぎりでは買わない。
         点数も絞る。広げるほど1点あたりが薄くなり、当たっても回収が細る。 */
      const pts = cand.filter(pt => pt.ev >= SANRENTAN_EV)
                      .sort((x, y) => y.ev - x.ev)
                      .slice(0, SANRENTAN_MAX);
      if(pts.length){
        const hit = pts.reduce((s, pt) => s + pt.hit, 0);
        const best = pts[0];
        push({name: "三連単", prio: 30, ratio: up.level === "high" ? 0.22 : 0.14,
              combos: pts.map(pt => pt.combo),
              hit: hit, needOdds: need(hit, pts.length), points: pts,
              cut: cand.length - pts.length,
              memo: `推定期待値の高い並びだけ ${pts.length}点（最良 ${best.combo} で ${best.ev.toFixed(2)}）。` +
                    `的中 ${(hit*100).toFixed(1)}%`});
      }
    }

    if(value && value.h.num !== a && n >= 4){
      const hit = wideProb(probs, idx[a], idx[value.h.num]);
      const mkHit = wideProb(mkts, idx[a], idx[value.h.num]);
      if(comboEv(hit, mkHit, "wide") >= BUY_EV)
      push({name: "ワイド（妙味）", prio: 40, ratio: 0.10, combos: [`${a}-${value.h.num}`],
            hit: hit, needOdds: need(hit, 1),
            evEst: comboEv(hit, mkHit, "wide"),
            points: withPoints([`${a}-${value.h.num}`], [hit], [mkHit], "wide"),
