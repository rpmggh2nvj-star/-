...
// 々（佐々木）・〆・ヶ を漢字と同じ扱いにする。外すと「佐々木」が人名として通らない。
const KANJI = "一-龥々〆ヶ";
const PERSON_RE = new RegExp("^[" + KANJI + "ぁ-んァ-ヴー]{2,6}$");
...
function parseNetkeiba(text){
  ...
  /* 母名の行。「母名 (母父名)」の形。外国産馬は「Sea Chanter (War Chant)」
     のようにローマ字で載るため、カタカナに限定しない。 */
  const isDam    = c => c.length > 0 && /^[ァ-ヴA-Za-z][^()\t]{1,24}\(/.test(c[0]);
  ...
  anchors.forEach(i => {
    ...
    const re = new RegExp("([" + KANJI + "ぁ-んァ-ヴー]{2,6})\\s*(?:4[7-9]|5\\d|6[0-3])(?:\\.\\d)?(?![\\d.])", "g");
    let m;
  ...
  anchors.forEach((i, idx) => {
    ...
    /* 母名・騎手の行はアンカーのすぐ上。母父が併記されていない馬や、
       母名がローマ字の外国産馬でも取りこぼさないよう、直上の行で拾い直す。 */
    let C = findIn(above, isDam);
    if(!C && i - 1 > prevAnchor && cells[i-1] !== A && cells[i-1] !== B) C = cells[i-1];
  ...
  if(grew.grown) warnings.push(`騎手名が紙面で切れていた ${grew.grown}頭 は、過去走の欄に載っている長い表記に直しました。`);
  if(grew.ambiguous.length){
    // 「山本聡」が山本聡紀にも山本聡哉にも当てはまる、といった場合。別人にしない。
    warnings.push("騎手名が紙面で切れており、候補が複数あるためそのままにしました: " +
      grew.ambiguous.map(a => `${a.num}番「${a.name}」（${a.cand.join("／")}）`).join("、") +
      "。同じ表記でも別の騎手のことがあるので、正しい方に直してください。");
  }
...
function expandJockeyNames(horses, pool){
  ...
  horses.forEach(h => {
    ...
    if(cand.length === 1){ h.jockeyName = cand[0]; grown++; }
    else if(cand.length > 1) ambiguous.push({num: h.num, name: name, cand: cand});
  });
  return {grown: grown, ambiguous: ambiguous};
}
...
function parseRowwise(text){
  ...
  found.forEach(f => {
    ...
    const jk = seg.match(new RegExp("(?:^|[\\s\\t])([" + KANJI + "ぁ-んァ-ヴー]{2,6})(?=[\\s\\t]|$)", "g"));
    if(jk){
